...
function mergeAgents(text:string):string{
  const start=text.indexOf(AGENTS_START), end=text.indexOf(AGENTS_END);
  if(start>=0&&end>=start) return `${text.slice(0,start).trimEnd()}${text.slice(0,start).trim()?"\n\n":""}${AGENTS_BLOCK}${text.slice(end+AGENTS_END.length)}`.trimEnd()+"\n";
  const prefix=text.trimEnd(); return `${prefix}${prefix?"\n\n":""}${AGENTS_BLOCK}\n`;
}
...
function enableHooksToml(text:string):{text:string;changed:boolean}{
  ...
  if(features<0){ const prefix=text.trimEnd(); return {text:`${prefix}${prefix?"\n\n":""}[features]\nhooks = true\n`,changed:true}; }
  let end=lines.length;
  for(let i=features+1;i<lines.length;i++) if(/^\s*\[.*\]\s*$/.test(lines[i])) { end=i; break; }
  for(let i=features+1;i<end;i++) if(/^\s*hooks\s*=/.test(lines[i])) { lines[i]="hooks = true"; return {text:lines.join("\n"),changed:true}; }
  lines.splice(features+1,0,"hooks = true");
  return {text:lines.join("\n"),changed:true};
}

export class CodexHostInstaller {
  async install(repositoryRoot:string,cliEntryPath:string,mcpServerPath:string,registerMcp=true):Promise<CodexInstallResult>{
    const codexDir=join(repositoryRoot,".codex"); await mkdir(codexDir,{recursive:true});
    ...
    if(!hooks||typeof hooks!=="object") hooks={}; if(!hooks.hooks||typeof hooks.hooks!=="object") hooks.hooks={};
    ...
    for(const event of ["SessionStart","UserPromptSubmit","Stop","SessionEnd"]){
      ...
      const present=existing.some((group:any)=>Array.isArray(group?.hooks)&&group.hooks.some((hook:any)=>hook?.type==="command"&&hook?.command===command));
      if(!present){ existing.push(hookEntry(command)); hooks.hooks[event]=existing; hookInstalled=true; }
    }
    await writeFile(hooksPath,JSON.stringify(hooks,null,2)+"\n","utf8");
    let config=""; try{config=await readFile(configPath,"utf8");}catch{}
    const enabled=enableHooksToml(config); if(enabled.changed||!config) await writeFile(configPath,enabled.text,"utf8");
    ...
    const warnings:string[]=[]; let mcp:CodexInstallResult["mcp"]="skipped";
    if(registerMcp){
      try{
        try { await execFileAsync("codex",["mcp","get","continuum"],{encoding:"utf8"}); mcp="existing"; }
        catch {
          await execFileAsync("codex",["mcp","add","continuum","--","node",resolve(mcpServerPath),resolve(cliEntryPath)],{encoding:"utf8"}); mcp="installed";
        }
      }catch(error){ mcp="unavailable"; warnings.push(`Could not register Codex MCP server: ${error instanceof Error?error.message:String(error)}`); }
    }
