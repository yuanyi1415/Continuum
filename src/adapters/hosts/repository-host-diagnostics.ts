...
export class RepositoryHostDiagnostics implements HostDiagnosticsPort {
  constructor(
  ...
  async inspect(): Promise<HostDiagnostic[]> {
    ...
    if (adapterPresent && existsSync(this.ompAssetPath)) adapterCurrent = readFileSync(extensionPath, "utf8") === readFileSync(this.ompAssetPath, "utf8");
    out.push({
      host: "omp",
