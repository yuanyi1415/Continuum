...
import type { ChangeStatus } from "../domain/change/change.js";
import type { RelationRouting, RelationType } from "../domain/relation/relation.js";
...
const artifactTypes = new Set<ArtifactType>(["context","adr","spec","ticket","review","evidence","document","other"]);
const relationTypes = new Set<RelationType>(["belongs_to","blocked_by","governed_by","domain_context","evidence","supersedes"]);
const relationRouting = new Set<RelationRouting>(["required","optional","historical"]);
const changeStatuses = new Set<ChangeStatus>(["active","closed","superseded"]);

function version(): string {
  return "Continuum 1.0.0";
}
...
async function main() {
  ...
  const command = args[0]; const json = has(args, "--json");
  const app = createApp(); const cwd = process.cwd();
  ...
  if (command === "init") {
    ...
    if (json) printJson({ ok:true, data, warnings:[], interaction:null }); else { console.log(`Initialized Continuum for ${result.project.name}`); console.log(`Project: ${result.project.projectId}`); console.log(`Snapshot: ${result.snapshot.snapshotId}`); }
    return;
  }
  if (command === "status") {
    const status = await app.status.execute(cwd);
    if (json) printJson({ ok:true, data:status, warnings: status.runtime.available?[]:["runtime.db is not present; durable project state remains available."], interaction:null }); else printStatus(status);
    return;
  }
  if (command === "doctor") {
  ...
  if (command === "migrate") {
    ...
    if(json) printJson({ok:true,data:result,warnings,interaction:null});
    else {
      ...
      if(result.durable.backupDirectory) console.log(`Backup: ${result.durable.backupDirectory}`);
    }
    return;
  }
  ...
  if (command === "archive") {
    if(args[1] === "verify") {
      ...
      if(json) printJson({ok:result.ok,data,warnings:result.failures,interaction:null});
      else {
    ...
    if(json) printJson({ok:true,data:{archive_path:result.archivePath,archive_id:result.manifest.archiveId,status:result.manifest.status,final_snapshot:result.manifest.finalSnapshot,revision:result.manifest.repository.revision,with_history:Boolean(result.manifest.historyBundlePath),verified:result.verified,checked_files:result.checkedFiles},warnings,interaction:null});
    else {
      ...
      if(result.manifest.historyBundlePath) console.log(`Git history: ${result.manifest.historyBundlePath}`);
    }
    return;
  }
  ...
  if (command === "change") {
    ...
    if (action === "open") {
      ...
      if (json) printJson({ ok:true, data:change, warnings:[], interaction:null }); else console.log(`Opened ${change.changeId} · ${change.title}`);
      return;
    }
    if (action === "list") {
      const rawStatus = flag(args, "--status");
      if (rawStatus && !changeStatuses.has(rawStatus as ChangeStatus)) throw new Error(`Invalid change status: ${rawStatus}`);
      const changes = await app.change.list.execute(cwd, rawStatus as ChangeStatus | undefined);
      if (json) printJson({ ok:true, data:changes, warnings:[], interaction:null }); else for (const change of changes) console.log(`${change.changeId}  ${change.status.padEnd(10)} ${change.title}`);
      return;
    }
    if (action === "show") {
      const change = await app.change.get.execute(cwd, requireArg(args[2], "Usage: continuum change show <change-id>"));
      if (json) printJson({ ok:true, data:change, warnings:[], interaction:null }); else console.log(JSON.stringify(change, null, 2));
      return;
    }
    if (action === "close") {
      ...
      if (json) printJson({ok:true,data:result,warnings:[],interaction:null});
      else console.log(`Closed ${result.change.changeId} · snapshot ${result.snapshot.snapshotId} · ${result.status.toLowerCase()}`);
      return;
    }
  ...
  if (command === "artifact") {
    ...
    if (action === "register") {
      ...
      const rawType = requireArg(flag(args, "--type"), "--type is required") as ArtifactType;
      if (!artifactTypes.has(rawType)) throw new Error(`Invalid artifact type: ${rawType}`);
      ...
      let relation: unknown = undefined;
      if (changeId) relation = await app.relation.register.execute(cwd, { from:result.artifact.artifactId, to:changeId, type:"belongs_to", routing:"required" });
      const data = { ...result, ...(relation ? { relation } : {}) };
      if (json) printJson({ ok:true, data, warnings:[], interaction:null }); else console.log(`${result.created?"Registered":"Found"} ${result.artifact.artifactId} · ${result.artifact.locator}`);
      return;
    }
    if (action === "list") {
      const artifacts = await app.artifact.list.execute(cwd);
      if (json) printJson({ ok:true, data:artifacts, warnings:[], interaction:null }); else for (const artifact of artifacts) console.log(`${artifact.artifactId}  ${artifact.type.padEnd(9)} ${artifact.locator}`);
      return;
    }
  ...
  if (command === "work") {
    ...
    if (action === "bind") {
      ...
      const sessionId = flag(args, "--session"); const host = flag(args, "--host");
      if ((sessionId && !host) || (!sessionId && host)) throw new Error("--session and --host must be provided together");
      ...
      if (json) printJson({ok:true,data:result,warnings:[],interaction:null});
      else console.log(`${result.created?"Bound":"Resumed"} ${result.binding.targetArtifactId} · baseline ${result.binding.workStartRevision}`);
      return;
    }
    if (action === "current") {
      const sessionId = flag(args, "--session"); const host = flag(args, "--host");
      ...
      if (json) printJson({ok:true,data:result,warnings:[],interaction:null});
      else if (result.binding) console.log(`Continuum · ${result.mode} · ${result.binding.targetArtifactId}${result.suppressed?" · suppressed":""}`); else console.log(`Continuum · ${result.mode}`);
      return;
    }
    if (action === "suppress-session") {
      ...
      if(json) printJson({ok:true,data,warnings:[],interaction:null}); else console.log(`Suppressed Continuum for ${host}/${sessionId}`);
      return;
    }
  ...
  if (command === "block") {
    ...
    if(action === "return-to-design") {
      ...
      if(json) printJson({ok:true,data:result,warnings:[],interaction:result.interaction});
      else console.log(`Returned to design · blocker ${result.interaction.id} remains active${result.releasedTargetArtifactId?` · released ${result.releasedTargetArtifactId}`:""}`);
      return;
    }
    if(action === "resolve") {
      ...
      if(json) printJson({ok:true,data:result,warnings:[],interaction:null});
      else console.log(`Resolved BLOCK ${result.requestId}`);
      return;
    }
  ...
  if (command === "matt") {
    ...
    if (action === "scan") {
      ...
      if (json) printJson({ok:true,data:result,warnings:[],interaction:null});
        ...
        if (result.registered.length) console.log(`Registered: ${result.registered.join(", ")}`);
        if (result.existing.length) console.log(`Existing: ${result.existing.join(", ")}`);
        if (result.linkedToChange.length) console.log(`Linked to change: ${result.linkedToChange.join(", ")}`);
      }
      return;
    }
  ...
  if (command === "context") {
    ...
    if(maxItems!==undefined && (!Number.isInteger(maxItems)||maxItems<=0)) throw new Error("--max-items must be a positive integer");
    if(maxTotalChars!==undefined && (!Number.isInteger(maxTotalChars)||maxTotalChars<=0)) throw new Error("--max-chars must be a positive integer");
    const result=await app.context.route.execute(cwd,workId,{maxItems,maxTotalChars});
    if(json) printJson({ok:true,data:result,warnings:result.warnings,interaction:null});
    else {
      console.log(`Context for ${result.workId}`);
      for(const item of result.required) console.log(`${item.priority}  required   ${item.artifactId.padEnd(14)} ${item.reason} · ${item.locator}`);
      for(const item of result.optional) console.log(`${item.priority}  optional   ${item.artifactId.padEnd(14)} ${item.reason} · ${item.locator}`);
      if(result.excludedHistorical.length) console.log(`Historical excluded: ${result.excludedHistorical.join(", ")}`);
  ...
  if (command === "reconcile") {
    ...
    if (changeId) {
      const result = await app.reconcile.change.execute(cwd, changeId);
      if (json) printJson({ok:true,data:result,warnings:[],interaction:null});
      else console.log(`Continuum · change reconcile · ${result.change.changeId} · snapshot ${result.snapshot.snapshotId} · ${result.status.toLowerCase()}`);
      return;
    }
    const result = await app.reconcile.work.execute(cwd);
    if (json) printJson({ok:true,data:result,warnings:[],interaction:null});
    else if (result.status === "NOOP") console.log("Continuum · reconcile · no changes");
    else if (result.status === "DEDUPED") console.log(`Continuum · reconcile · deduped · ${result.candidate?.currentRevision ?? ""}`);
    else console.log(`Continuum · reconcile · ${result.candidate?.knowledgeImpact} · ${result.candidate?.changedFiles.length ?? 0} files`);
    return;
  }
  ...
  if (command === "lifecycle") {
    ...
    if(action === "checkpoint") {
      ...
      if(json) printJson({ok:true,data:result,warnings:[],interaction:null});
      else console.log(`Continuum · checkpoint · ${result.status.toLowerCase()}`);
      return;
    }
    if(action === "install-git-hook") {
      const result=await app.lifecycle.installGitHook(cwd,flag(args,"--command"));
      if(json) printJson({ok:true,data:result,warnings:[],interaction:null}); else console.log(`${result.updated?"Installed":"Found"} ${result.path}`);
      return;
    }
  ...
  if (command === "host") {
    ...
    if(hostAction === "codex") {
      ...
      if(action === "capabilities") {
        ...
        if(json) printJson({ok:true,data:result,warnings:result.diagnostics,interaction:null}); else console.log(JSON.stringify(result,null,2));
        return;
      }
      if(action === "hook") {
        ...
        try {
          ...
          if(error instanceof ContinuumError && error.code === "CONTINUUM_PROJECT_NOT_FOUND") { process.stdout.write(JSON.stringify({continue:true,suppressOutput:true})); return; }
          if(error instanceof ContinuumError) { process.stdout.write(JSON.stringify({continue:false,decision:"block",reason:`${error.code}: ${error.message}`,hookSpecificOutput:{hookEventName:String(payload.hook_event_name ?? "UserPromptSubmit")}})); return; }
          throw error;
        }
        return;
      }
      if(action === "interaction") {
        ...
        if(interactionAction === "next") {
          ...
          if(rawType && rawType!=="DECISION" && rawType!=="BLOCK") throw new Error("--type must be DECISION or BLOCK");
          const result=await app.interaction.pending.execute(cwd,rawType as "DECISION"|"BLOCK"|undefined);
          if(json) printJson({ok:true,data:result,warnings:[],interaction:result}); else console.log(result?JSON.stringify(result,null,2):"No pending Continuum interaction.");
          return;
        }
        if(interactionAction === "resolve") {
          const interactionId=requireArg(args[4],"Usage: continuum host codex interaction resolve <id>");
          const rawAction=(flag(args,"--action") ?? "accept") as "accept"|"cancel"|"dismiss"|"unavailable"|"resolve";
          if(!["accept","cancel","dismiss","unavailable","resolve"].includes(rawAction)) throw new Error("Invalid --action");
          ...
          if(json) printJson({ok:true,data:result,warnings:[],interaction:null}); else console.log(`Resolved ${result.requestId} · ${result.action}${result.selectedOption?` · ${result.selectedOption}`:""}`);
          return;
        }
    ...
    if(hostAction === "omp") {
      ...
      if(action === "event") {
        ...
        try {
          ...
          if(json) printJson({ok:true,data:result,warnings:[],interaction:result.interaction??null}); else process.stdout.write(JSON.stringify(result));
        } catch(error) {
          if(error instanceof ContinuumError && error.code === "CONTINUUM_PROJECT_NOT_FOUND") { if(json) printJson({ok:true,data:{state:"SILENT"},warnings:[],interaction:null}); else process.stdout.write(JSON.stringify({state:"SILENT"})); return; }
          throw error;
        }
        return;
      }
  ...
  if (command === "relation") {
    const action = args[1];
    if (action === "add") {
      const from=requireArg(args[2],"relation source is required"); const rawType=requireArg(args[3],"relation type is required") as RelationType; const to=requireArg(args[4],"relation target is required");
      if(!relationTypes.has(rawType)) throw new Error(`Invalid relation type: ${rawType}`);
      const rawRouting=(flag(args,"--routing") ?? "required") as RelationRouting; if(!relationRouting.has(rawRouting)) throw new Error(`Invalid relation routing: ${rawRouting}`);
      const result=await app.relation.register.execute(cwd,{from,to,type:rawType,routing:rawRouting});
      if(json) printJson({ok:true,data:result,warnings:[],interaction:null}); else console.log(`${result.created?"Added":"Found"} ${result.relation.relationId} · ${from} -[${rawType}]-> ${to}`);
      return;
    }
    if (action === "list") {
      const relations=await app.relation.list.execute(cwd,flag(args,"--for"));
      if(json) printJson({ok:true,data:relations,warnings:[],interaction:null}); else for(const relation of relations) console.log(`${relation.relationId}  ${relation.from} -[${relation.type}/${relation.routing}]-> ${relation.to}`);
      return;
    }
...
main().catch(error => {
  if (error instanceof ContinuumError) {
    const interaction = error.details && typeof error.details === "object" && "interaction" in error.details ? (error.details as any).interaction : null;
    ...
    if (process.argv.includes("--json")) printJson(payload); else console.error(`${error.code}: ${error.message}`);
    process.exitCode = 2;
  } else {
    if (process.argv.includes("--json")) printJson({ ok:false, error:{ code:"CONTINUUM_INTERNAL", message:error instanceof Error?error.message:String(error), recoverable:false } });
