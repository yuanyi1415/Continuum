export function printJson(data) { process.stdout.write(JSON.stringify(data, null, 2) + "\n"); }
export function printStatus(status) {
    ...
    if (status.work?.binding)
        console.log(`Work:     ${status.work.binding.targetArtifactId} (${status.work.binding.workId})`);
    if (status.changes?.active?.length) {
        console.log("Active changes:");
        for (const change of status.changes.active)
            console.log(`  ${change.changeId}  ${change.title}`);
    }
}
