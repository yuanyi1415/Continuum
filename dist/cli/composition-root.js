...
export function createApp() {
    ...
    const stores = (root) => new YamlProjectStore(root);
    const runtimes = (root) => new SqliteRuntimeStore(root, () => clock.nowIso());
    ...
    return {
        ...
        lifecycle: {
            checkpoint,
            installGitHook: async (cwd, command) => gitHookInstaller.install(await git.getHooksDirectory(cwd), command),
        },
    };
}
