...
import { cpSync, existsSync, lstatSync, mkdirSync, readFileSync, readlinkSync, readdirSync, renameSync, rmSync, unlinkSync, writeFileSync, } from "node:fs";
...
const COMPLETENESS_TYPES = new Set(["context", "adr", "spec", "ticket", "review", "evidence"]);
...
function gitBlob(repositoryRoot, revision, locator) {
    if (!locator || locator.includes("\0") || locator.startsWith("/"))
        return undefined;
    ...
    try {
        return execFileSync("git", ["show", `${revision}:${rel}`], { cwd: repositoryRoot, encoding: "buffer", maxBuffer: 64 * 1024 * 1024, stdio: ["ignore", "pipe", "ignore"] });
    }
    catch {
        return undefined;
    }
}
function gitBlobId(repositoryRoot, revision, locator) {
    if (!locator || locator.includes("\0") || locator.startsWith("/"))
        return undefined;
    const absolute = resolve(repositoryRoot, locator);
    if (!pathInside(repositoryRoot, absolute))
    ...
    const rel = slash(relative(repositoryRoot, absolute));
    try {
        return execFileSync("git", ["rev-parse", `${revision}:${rel}`], { cwd: repositoryRoot, encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] }).trim() || undefined;
    }
    catch {
        return undefined;
    }
}
function listHashable(root, current = root) {
    const out = [];
    for (const entry of readdirSync(current, { withFileTypes: true })) {
        const full = join(current, entry.name);
        if (entry.isDirectory())
            out.push(...listHashable(root, full));
...
function manifestFromDoc(doc) {
    if (!doc || typeof doc !== "object" || doc.schema_version !== 1 || typeof doc.archive_id !== "string")
        return undefined;
    try {
        return {
            schemaVersion: 1,
            ...
            changes: Array.isArray(doc.changes) ? doc.changes.map((change) => ({
            ...
            artifacts: Array.isArray(doc.artifacts) ? doc.artifacts.map((item) => ({
...
export class LocalProjectArchive {
    async create(repositoryRoot, input) {
        const archiveRoot = join(repositoryRoot, ".continuum", "archives");
        const target = join(archiveRoot, input.archiveId);
        if (existsSync(target))
            throw new ContinuumError("CONTINUUM_ARCHIVE_EXISTS", `Archive already exists: ${input.archiveId}`, false);
        ...
        try {
            ...
            if (requiredFailures.length && !input.allowIncomplete) {
                throw new ContinuumError("CONTINUUM_ARCHIVE_INCOMPLETE", "Archive cannot be self-contained because required artifacts could not be materialized. Re-run with --allow-incomplete to preserve an incomplete archive explicitly.", true, { artifacts: requiredFailures.map(item => ({ artifactId: item.artifactId, status: item.status, locator: item.locator })) });
            }
            ...
            let historyBundlePath;
            if (input.withHistory) {
                const bundle = join(staging, "source", "history.bundle");
                try {
                    execFileSync("git", ["bundle", "create", bundle, "--all"], { cwd: repositoryRoot, stdio: "pipe" });
                }
                catch (error) {
                    throw new ContinuumError("CONTINUUM_ARCHIVE_HISTORY_FAILED", "Cannot create Git history bundle.", false, { cause: error instanceof Error ? error.message : String(error) });
                }
            ...
            if (!verification.ok)
                throw new ContinuumError("CONTINUUM_ARCHIVE_VERIFY_FAILED", "Newly created archive failed its own integrity verification.", false, { failures: verification.failures });
    ...
    async verify(archivePath) {
        ...
        if (!existsSync(root) || !lstatSync(root).isDirectory())
            return { ok: false, archivePath: root, checkedFiles: 0, failures: ["Archive directory does not exist."] };
        let manifest;
        const manifestPath = join(root, "manifest.yaml");
        if (!existsSync(manifestPath))
            failures.push("manifest.yaml is missing.");
        else {
            try {
                const doc = YAML.parse(readFileSync(manifestPath, "utf8"));
                manifest = manifestFromDoc(doc);
                if (!manifest)
                    failures.push("manifest.yaml is invalid or unsupported.");
                else {
                    try {
                        assertArchiveManifestInvariant(manifest);
                    }
                    catch (error) {
                        failures.push(`manifest invariant failed: ${error instanceof Error ? error.message : String(error)}`);
                    }
                }
            }
            catch (error) {
                failures.push(`manifest.yaml cannot be parsed: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        const checksumsPath = join(root, "checksums.sha256");
        if (!existsSync(checksumsPath))
            return { ok: false, archivePath: root, archiveId: manifest?.archiveId, status: manifest?.status, checkedFiles: 0, failures: [...failures, "checksums.sha256 is missing."] };
        ...
        for (const [relPath, hash] of expected) {
            ...
            if (!pathInside(root, full)) {
                failures.push(`Checksum path escapes archive: ${relPath}`);
                continue;
            }
            if (!existsSync(full)) {
                failures.push(`Missing archived file: ${relPath}`);
                continue;
            }
            try {
                const actual = hashPath(full);
                if (actual !== hash)
                    failures.push(`Checksum mismatch: ${relPath}`);
                checked += 1;
            }
            catch (error) {
                failures.push(`Cannot verify ${relPath}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        const actualFiles = new Set(listHashable(root).map(path => slash(relative(root, path))).filter(path => path !== "checksums.sha256"));
        for (const file of actualFiles)
            if (!expected.has(file))
                failures.push(`Unlisted archived file: ${file}`);
        for (const file of expected.keys())
            if (!actualFiles.has(file))
                failures.push(`Checksum references missing file: ${file}`);
...
//# sourceMappingURL=local-project-archive.js.map