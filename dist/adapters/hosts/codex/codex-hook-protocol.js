function eventName(payload) { return String(payload.hook_event_name ?? ""); }
export function decodeCodexHook(payload, defaultCwd) {
    ...
    if (event === "SessionStart")
        return { host: "codex", event: "SESSION_START", sessionId, cwd, source: "codex:SessionStart" };
    if (event === "UserPromptSubmit")
        return { host: "codex", event: "USER_PROMPT", sessionId, cwd, prompt: String(payload.prompt ?? ""), source: "codex:UserPromptSubmit" };
    if (event === "Stop")
        return { host: "codex", event: "STABLE_CHECKPOINT", sessionId, cwd, source: "codex:Stop" };
    if (event === "SessionEnd")
        return { host: "codex", event: "SESSION_END", sessionId, cwd, source: "codex:SessionEnd" };
    return null;
}
...
export function encodeCodexHook(payload, result) {
...
//# sourceMappingURL=codex-hook-protocol.js.map