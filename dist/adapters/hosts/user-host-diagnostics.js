...
export class UserHostDiagnostics {
    ompAssetPath;
    repositoryRoot;
    detectCodex;
    detectOmp;
    home;
    env;
    constructor(ompAssetPath, repositoryRoot, detectCodex = () => new CodexCapabilityDetector().detect(), detectOmp = () => new OmpCapabilityDetector().detect(), home = homedir, env = process.env) {
    ...
    codexHome() { return resolve(this.env.CODEX_HOME?.trim() || join(this.home(), ".codex")); }
    ompAgentDir() { return resolve(this.env.PI_CODING_AGENT_DIR?.trim() || join(this.home(), ".omp", "agent")); }
    async inspect() {
        const out = [];
        ...
        let adapterCurrent;
        if (globalOmpPresent && existsSync(this.ompAssetPath))
...
//# sourceMappingURL=user-host-diagnostics.js.map