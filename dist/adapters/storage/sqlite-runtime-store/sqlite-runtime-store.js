...
function openDatabase(path) {
    try {
        ...
        return { db: new Ctor(path), driver: "better-sqlite3" };
    }
    catch (error) {
...
function workBindingFromRow(row) {
    return {
        workId: row.work_id,
...
function sessionBindingFromRow(row) {
...
export class SqliteRuntimeStore {
    ...
    async health() {
        ...
        try {
            ...
            try {
                ...
                if (quickValue !== "ok")
                    return { available: false, state: "corrupt", driver, error: `SQLite quick_check: ${String(quickValue)}` };
                const version = this.currentVersion(db);
                if (version > RUNTIME_SCHEMA_VERSION)
                    return { available: false, state: "too-new", schemaVersion: version, driver, error: `runtime.db schema ${version} is newer than supported ${RUNTIME_SCHEMA_VERSION}.` };
                if (version < RUNTIME_SCHEMA_VERSION)
                    return { available: true, state: "needs-migration", schemaVersion: version, driver };
            ...
            if (error instanceof ContinuumError && error.code === "CONTINUUM_RUNTIME_DRIVER_MISSING")
                return { available: false, state: "unavailable", error: error.message };
            return { available: false, state: "corrupt", error: error instanceof Error ? error.message : String(error) };
        }
    }
    async recover() {
        const health = await this.health();
        if (health.state === "too-new")
            throw new ContinuumError("CONTINUUM_SCHEMA_TOO_NEW", health.error ?? "runtime.db schema is newer than this CLI.", false, { schemaVersion: health.schemaVersion, supported: RUNTIME_SCHEMA_VERSION });
        if (health.state === "unavailable")
            throw new ContinuumError("CONTINUUM_RUNTIME_DRIVER_MISSING", health.error ?? "Runtime driver is unavailable.", true);
        ...
        if (health.state === "healthy")
            return { action: "none", schemaVersion: RUNTIME_SCHEMA_VERSION, quarantinedPaths: [] };
    ...
    async getWorktreeBinding(worktreeId) {
        if (!existsSync(this.path))
            return null;
        const { db } = openDatabase(this.path);
        try {
    ...
    async listWorktreeBindings() {
        if (!existsSync(this.path))
            return [];
        const { db } = openDatabase(this.path);
        try {
            const rows = db.prepare("SELECT * FROM worktree_bindings ORDER BY bound_at, worktree_id").all?.() ?? [];
            return rows.map((row) => workBindingFromRow(row));
        }
        finally {
            db.close();
        }
    }
    async bindWork(binding) {
        ...
        worktree_id, work_id, target_artifact_id, change_id, work_start_revision, bound_at, binding_source
            ...
            if (!row)
                throw new ContinuumError("CONTINUUM_RUNTIME_CORRUPT", "Work binding could not be persisted.");
    ...
    async clearWorktreeBinding(worktreeId) {
        if (!existsSync(this.path))
            return;
        const { db } = openDatabase(this.path);
        try {
            db.prepare("DELETE FROM worktree_bindings WHERE worktree_id = ?").run(worktreeId);
        }
        finally {
            db.close();
        }
    }
    async clearSessionBindingsForWorktree(worktreeId) {
        if (!existsSync(this.path))
            return;
        const { db } = openDatabase(this.path);
        try {
            db.prepare("DELETE FROM session_bindings WHERE worktree_id = ?").run(worktreeId);
        }
        finally {
            db.close();
        }
    }
    async getSessionBinding(sessionId, host) {
        if (!existsSync(this.path))
            return null;
        const { db } = openDatabase(this.path);
        try {
    ...
    async suppressSession(suppression) {
        await this.initialize();
        const { db } = openDatabase(this.path);
        try {
    ...
    async isSessionSuppressed(sessionId, host) {
        return (await this.getSessionSuppression(sessionId, host)) !== null;
    }
    async getSessionSuppression(sessionId, host) {
        if (!existsSync(this.path))
            return null;
        const { db } = openDatabase(this.path);
        try {
    ...
    async loadPendingReconcile(workId) {
        if (!existsSync(this.path))
            return null;
        const { db } = openDatabase(this.path);
        try {
            const row = db.prepare("SELECT * FROM pending_reconciles WHERE work_id = ?").get(workId);
    ...
    async savePendingReconcile(candidate) {
        await this.initialize();
        const { db } = openDatabase(this.path);
        try {
      ...
      ON CONFLICT(work_id) DO UPDATE SET
        ...
        updated_at = excluded.updated_at`)
                .run(candidate.workId, candidate.baseRevision, candidate.currentRevision, JSON.stringify(candidate.changedFiles), JSON.stringify(candidate.evidence), candidate.knowledgeImpact, JSON.stringify(candidate.actions), candidate.updatedAt);
        }
        finally {
            db.close();
        }
    }
    async deletePendingReconcile(workId) {
        if (!existsSync(this.path))
            return;
        const { db } = openDatabase(this.path);
        try {
            db.prepare("DELETE FROM pending_reconciles WHERE work_id = ?").run(workId);
        }
        finally {
            db.close();
        }
    }
    async savePendingInteraction(request) {
        await this.initialize();
        const { db } = openDatabase(this.path);
        try {
    ...
    async loadPendingInteraction(interactionId) {
        if (!existsSync(this.path))
            return null;
        const { db } = openDatabase(this.path);
        try {
            const row = db.prepare("SELECT payload_json FROM pending_interactions WHERE interaction_id = ? AND status = 'pending'").get(interactionId);
    ...
    async resolveInteraction(result) {
        if (!existsSync(this.path))
            return;
        const { db } = openDatabase(this.path);
        try {
    ...
    async hasIdempotencyKey(key) {
        ...
        const { db } = openDatabase(this.path);
        try {
            return Boolean(db.prepare("SELECT 1 AS ok FROM idempotency_keys WHERE key = ?").get(key));
        }
        finally {
            db.close();
        }
    }
    async saveIdempotencyKey(key, createdAt) {
        await this.initialize();
        const { db } = openDatabase(this.path);
        try {
            db.prepare("INSERT OR IGNORE INTO idempotency_keys(key, created_at) VALUES (?, ?)").run(key, createdAt);
        }
        finally {
            db.close();
        }
