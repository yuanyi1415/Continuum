...
export class ShellGitPort {
    async inspect(cwd) {
        const root = await git(cwd, ["rev-parse", "--show-toplevel"]);
        if (!root)
            throw new ContinuumError("CONTINUUM_GIT_UNAVAILABLE", "Current directory is not inside a Git repository.");
        const canonicalRoot = await realpath(root);
        const head = await git(canonicalRoot, ["rev-parse", "--verify", "HEAD"], true);
        if (!head)
            throw new ContinuumError("CONTINUUM_GIT_UNBORN", "Continuum requires at least one Git commit so the initial Project Snapshot has a stable code baseline.", true);
        const rootRevision = (await git(canonicalRoot, ["rev-list", "--max-parents=0", "HEAD"]))?.split(/\s+/)[0];
        if (!rootRevision)
            throw new ContinuumError("CONTINUUM_GIT_UNAVAILABLE", "Cannot determine Git root revision.");
    ...
    async isAncestor(cwd, ancestorRevision, descendantRevision) {
        const facts = await this.inspect(cwd);
        try {
...
//# sourceMappingURL=shell-git-port.js.map