...
export class Doctor {
    ...
    hostFactory;
    reconcileWork;
    constructor(git, storeFactory, runtimeFactory, durableFactory, authorityFactory, hostFactory, reconcileWork) {
        this.git = git;
    ...
    async execute(cwd, options = {}) {
        ...
        if (durableUsable) {
            ...
            try {
                ...
                let snapshot;
                try {
                    snapshot = await store.loadSnapshot(current.snapshotId);
                }
                catch (error) {
                ...
                const missingChangeRefs = [];
                for (const change of changes)
                    for (const ref of [...change.specRefs, ...change.ticketRefs])
                        if (!artifactIds.has(ref))
                            missingChangeRefs.push({ changeId: change.changeId, artifactId: ref });
                ...
                if (missingChangeRefs.length || orphanRelations.length) {
                    ...
                    checks.push({ name: "artifact-graph", level: "PASS", message: `Artifact graph references are complete (${artifacts.length} artifacts, ${relations.length} relations).` });
                const authority = this.authorityFactory(root);
                const missingAuthority = [];
                for (const artifact of artifacts) {
                    try {
                        if (!(await authority.exists(artifact)))
                            missingAuthority.push({ artifactId: artifact.artifactId, locator: artifact.locator, authority: artifact.authority });
                    }
                    catch (error) {
                        missingAuthority.push({ artifactId: artifact.artifactId, locator: artifact.locator, authority: artifact.authority, error: error instanceof Error ? error.message : String(error) });
                    }
                }
                if (missingAuthority.length)
                    checks.push({ name: "artifact-authority", level: "FAIL", code: "CONTINUUM_AUTHORITY_UNAVAILABLE", message: `${missingAuthority.length} ArtifactRef(s) cannot be resolved from their authority.`, details: { artifacts: missingAuthority } });
                else
                    checks.push({ name: "artifact-authority", level: "PASS", message: "All ArtifactRef authorities are available." });
            }
            catch (error) {
        ...
        if (options.recover && ["missing", "corrupt", "needs-migration"].includes(runtimeHealth.state ?? "")) {
            try {
                ...
                checks.push({ name: "runtime-recovery", level: "FAIL", code: error instanceof Error && "code" in error ? String(error.code) : "CONTINUUM_RUNTIME_CORRUPT", message: error instanceof Error ? error.message : String(error) });
            }
        }
        if (runtimeHealth.state === "healthy")
            checks.push({ name: "runtime", level: "PASS", message: `runtime.db schema=${runtimeHealth.schemaVersion} driver=${runtimeHealth.driver}` });
        else if (runtimeHealth.state === "needs-migration")
            checks.push({ name: "runtime", level: "WARN", recoverable: true, message: `runtime.db schema=${runtimeHealth.schemaVersion} needs migration; startup or continuum migrate can advance it.` });
        else if (runtimeHealth.state === "missing")
            checks.push({ name: "runtime", level: "WARN", recoverable: true, message: "runtime.db is missing; durable project state remains readable. Run continuum doctor --recover to recreate runtime state." });
        else if (runtimeHealth.state === "too-new")
            checks.push({ name: "runtime", level: "FAIL", code: "CONTINUUM_SCHEMA_TOO_NEW", message: runtimeHealth.error ?? "runtime.db is newer than this CLI." });
        else if (runtimeHealth.state === "corrupt")
            checks.push({ name: "runtime", level: "FAIL", code: "CONTINUUM_RUNTIME_CORRUPT", recoverable: true, message: runtimeHealth.error ?? "runtime.db is corrupt. Run continuum doctor --recover to quarantine and rebuild it." });
        else if (runtimeHealth.state === "unavailable")
            checks.push({ name: "runtime", level: "FAIL", code: "CONTINUUM_RUNTIME_DRIVER_MISSING", recoverable: true, message: runtimeHealth.error ?? "Runtime storage driver is unavailable." });
        if (durableUsable && store && runtimeHealth.available) {
            try {
                const bindings = await runtime.listWorktreeBindings();
                const stale = [];
                for (const binding of bindings) {
                    if (!(await store.hasArtifact(binding.targetArtifactId)) || (binding.changeId && !(await store.hasChange(binding.changeId))))
                        stale.push(binding.worktreeId);
                }
                if (stale.length && options.recover) {
                    for (const worktreeId of stale) {
                        await runtime.clearSessionBindingsForWorktree(worktreeId);
                        await runtime.clearWorktreeBinding(worktreeId);
                    }
                    recovered = true;
                    ...
                    checks.push({ name: "work-bindings", level: "WARN", recoverable: true, message: `${stale.length} Runtime Work Binding(s) reference missing Durable nodes.`, details: { worktreeIds: stale } });
                else
                    checks.push({ name: "work-bindings", level: "PASS", message: `${bindings.length} Runtime Work Binding(s) are structurally valid.` });
                ...
                if (currentBinding && currentBinding.workStartRevision !== facts.currentRevision) {
                    ...
                    if (!pending && options.recover) {
                        ...
                        checks.push({ name: "pending-work", level: "PASS", message: `Pending Work Reconcile already exists for ${currentBinding.targetArtifactId}.` });
                }
            }
            catch (error) {
        ...
        try {
            ...
            for (const host of hostDiagnostics)
                checks.push({ name: `host-${host.host}`, level: host.level, code: host.level === "WARN" ? "CONTINUUM_HOST_UNAVAILABLE" : undefined, recoverable: host.level === "WARN", message: host.message, details: host.details });
        }
        catch (error) {
