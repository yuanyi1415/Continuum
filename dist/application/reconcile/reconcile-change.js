...
function evidencePass(candidate) {
...
export class ReconcileChange {
    ...
    authorityFactory;
    clock;
    constructor(git, storeFactory, runtimeFactory, authorityFactory, clock) {
        this.git = git;
    ...
    async runtimeFacts(cwd, changeId, integrationRevision) {
        ...
        for (const root of roots) {
            ...
            for (const binding of bindings) {
                ...
                if (!candidate)
                    continue;
                if (!(await this.git.isAncestor(cwd, candidate.currentRevision, integrationRevision)))
                    continue;
    ...
    async validateSpecs(store, authority, change) {
        if (change.specRefs.length < 1)
            throw new ContinuumError("CONTINUUM_CHANGE_NOT_READY", `Change ${change.changeId} has no Spec ArtifactRef.`, true);
        for (const specId of change.specRefs) {
            const artifact = await store.loadArtifact(specId);
            if (artifact.type !== "spec")
                throw new ContinuumError("CONTINUUM_CHANGE_NOT_READY", `Change spec ref is not a Spec: ${specId}`, true);
            const resolved = await authority.resolve(artifact);
            if (!resolved.exists || !resolved.content?.trim())
                throw new ContinuumError("CONTINUUM_AUTHORITY_UNAVAILABLE", `Spec is unavailable or empty: ${specId}`, true);
        }
    }
    async designArtifactVersions(store, authority, change, affectedTickets, revision) {
        const artifactIds = new Set(change.specRefs);
        ...
        for (let depth = 0; depth < 2; depth += 1) {
            ...
            for (const from of frontier.splice(0, frontier.length)) {
                ...
                for (const relation of relations.filter(item => item.from === from && item.routing !== "historical")) {
                    if (relation.type !== "governed_by" && relation.type !== "domain_context" && relation.type !== "belongs_to")
                        continue;
                    if (!(await store.hasArtifact(relation.to)))
                        continue;
                    const artifact = await store.loadArtifact(relation.to);
                    if (artifact.type === "spec" || artifact.type === "adr" || artifact.type === "context")
    ...
    interaction(change, revision, type, now, reason, context) {
        const request = {
    ...
    async execute(cwd, changeId) {
        ...
        if (change.status !== "active")
            throw new ContinuumError("CONTINUUM_CHANGE_NOT_READY", `Change ${changeId} is ${change.status}; only active Changes can be reconciled.`, true);
        ...
        if (inputSnapshot.lastReconcile) {
            try {
                ...
                if (!(error instanceof ContinuumError) || error.code !== "CONTINUUM_RECONCILE_NOT_FOUND")
                    throw error;
            }
        }
        const authority = this.authorityFactory(gitFacts.root);
        await this.validateSpecs(store, authority, change);
        if (change.ticketRefs.length < 1)
            throw new ContinuumError("CONTINUUM_CHANGE_NOT_READY", `Change ${changeId} has no Ticket ArtifactRef.`, true);
        for (const ticketId of change.ticketRefs) {
            const ticket = await store.loadArtifact(ticketId);
            if (ticket.type !== "ticket")
                throw new ContinuumError("CONTINUUM_CHANGE_NOT_READY", `Change ticket ref is not a Ticket: ${ticketId}`, true);
        }
        const runtimeFacts = await this.runtimeFacts(cwd, changeId, gitFacts.currentRevision);
        ...
        for (const fact of runtimeFacts) {
            ...
            list.push(fact);
            grouped.set(fact.binding.targetArtifactId, list);
        }
        const resolvedWork = [];
        ...
        if (blocked) {
            ...
            const runtime = this.runtimeFactory(gitFacts.root);
...
//# sourceMappingURL=reconcile-change.js.map