...
export class ArchiveProject {
    ...
    clock;
    ids;
    constructor(git, storeFactory, archive, clock, ids) {
        this.git = git;
    ...
    async execute(cwd, input = {}) {
        ...
        if (activeChanges.length || finalSnapshot.activeChanges.length) {
            throw new ContinuumError("CONTINUUM_ARCHIVE_NOT_READY", "Project Archive requires all Changes to be closed or superseded.", true, { activeChanges: activeChanges.map(change => change.changeId), snapshotActiveChanges: finalSnapshot.activeChanges });
        }
        ...
        if (!baseline) {
            throw new ContinuumError("CONTINUUM_ARCHIVE_NOT_READY", "Final Snapshot does not contain a baseline for the Project repository identity.", false, { repositoryIdentity: project.repository.identity, baselines: finalSnapshot.baselines });
        }
        if (!(await this.git.isAncestor(facts.root, baseline.revision, facts.currentRevision))) {
            throw new ContinuumError("CONTINUUM_ARCHIVE_NOT_READY", "Final Snapshot revision is not reachable from the current repository; refusing to materialize an ambiguous archive.", false, { finalRevision: baseline.revision, currentRevision: facts.currentRevision });
        }
