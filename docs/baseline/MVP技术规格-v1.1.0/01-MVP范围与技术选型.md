...
15. Project Archive；
16. Doctor / Recovery；
17. Global Host Setup / Project Auto Detection；
18. Agent-driven Project Operations。
...
continuum setup
+
每个项目仅 continuum init / Agent-driven init
```

Host Bridge 属于 User / Machine Scope，不再默认复制到每个 Repository。Global Bridge 必须根据当前 cwd 向上查找 `.continuum/project.yaml`；未找到时 silent NOOP。
