...
## 10. Archive Port（v1.0.6）

Core Application 负责判断 Project 是否可 Archive，并冻结 Final Snapshot / Final Revision。

Archive Port 负责物理物化：

```text
create(repository_root, request)
verify(archive_path)
```

Adapter 可以调用 Git / filesystem；Application 不直接依赖这些实现细节。
